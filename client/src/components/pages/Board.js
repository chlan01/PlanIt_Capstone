...
const Board = ({ match }) => {
  ...
  useEffect(() => {
    if (board?.title) document.title = board.title + ' | PlanIt';
  }, [board?.title]);
    ...
    <div
      ...
      style={{
        backgroundImage:
          ...
          (board.backgroundURL
            ? board.backgroundURL
            : 'https://images.unsplash.com/photo-1579033461380-adb47c3eb938?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1000&q=80') +
          ')',
      }}
    >
      <Navbar />
